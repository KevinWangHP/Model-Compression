...
def plot_bin_hist(net, optimizer, val_loader, dirname, device, criterion, epoch):
    from .quantize import config
    if config.backward_persample:
        dirname = dirname + "/PSQ/" + str(config.backward_num_bits) + "bit"
    else:
        dirname = dirname + "/PTQ/" + str(config.backward_num_bits) + "bit"
    ...
    for i in [1, 2]:
        ...
        fig.savefig(dirname + '/{}_hist_epoch'.format(i) + f'{epoch:>03}' + '.png')
    ...
    def plot_each(preconditioner, Preconditioner, name, g):
        ...
        ax.set_position([l + 0.05 * w, b, 0.95 * w, h])
        # ax.set_ylim([0, 128])
        fig.savefig(dirname + '/{}_bin_size_hist_epoch'.format(name) + f'{epoch:>03}' + '.png')
        ...
        print(f'\n\nEpoch{epoch:>03} Gradient Variance of {name}: {var}')

    from .preconditioner import ScalarPreconditionerAct, DiagonalPreconditioner, BlockwiseHouseholderPreconditioner
    if config.backward_persample:
